...
class MsgLogsController < ApplicationController
  ...
  def	ajax_search
		...
		pattern = message.empty? ? ".*" : message.gsub("#", ".*?").gsub("(", '\(').gsub(")", '\)').gsub("]", '\]').gsub("[", '\[').gsub(/\s+/, "")
		page = params[:page].to_i
		page_limit = params[:page_limit].to_i
		detailed_messages = []

		total_count = MsgLog.where("message REGEXP ? AND recorded_at < ?", "#{pattern}", collected_at).count
		
		msg_logs = MsgLog.where("message REGEXP ? AND recorded_at < ?", "#{pattern}", collected_at).limit(page_limit).offset((page - 1) * page_limit)
